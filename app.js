...
// ====== 旧データ統合（ワンショット移行） ======
// 過去に異なる端末で p_ext_<timestamp>_<index> 形式の重複IDが作られてしまったデータを、
// 決定的ID(p_form_<hash>) に統合し、完全な状態でクラウドに push する。
async function migrateLegacyData() {
    const warn =
        '【旧データを新形式に統合します】\n\n' +
        '・同じ生徒が複数の p_ext_ IDで重複している場合、1つの p_form_ IDに統合します\n' +
        '・成績・プルダウン・備考は「非空優先」でマージされます\n' +
        '・旧 p_ext_ エントリは削除されます\n' +
        '・実行前に現在のクラウドデータを自動でJSONバックアップ保存します\n' +
        '・この操作は戻せません（バックアップから手動復元は可能）\n\n' +
        '続行しますか？';
    if (!confirm(warn)) return;

    showLoading('クラウドから最新データを取得中...');
    try {
        // 1. クラウドの最新状態を取得
        const res = await fetch(GAS_WEB_APP_URL + '?t=' + Date.now());
        const data = await res.json();
        const cloudAppData = data.appData || {};
        const formResponses = Array.isArray(data.formResponses) ? data.formResponses : [];

        // 2. バックアップを自動ダウンロード
        const backup = {
            exportedAt: new Date().toISOString(),
            source: GAS_WEB_APP_URL,
            data: data
        };
        ...
        backupLink.download = `eiken_cloud_backup_${tsStr}.json`;
        ...
        // 3. フォーム回答から index→name, name→newId マップを作成
        const indexToName = {};
        const nameToNewId = {};
        const nameToMeta = {};
        formResponses.forEach((row, i) => {
            const keys = Object.keys(row);
            const nameKey = keys.find(k => k.includes('氏名'));
            const yearKey = keys.find(k => k.includes('学年'));
            const gradeKey = keys.find(k => k.includes('級'));
            const tabletKey = keys.find(k => k.includes('タブレット'));
            if (!nameKey) return;
            const rawName = String(row[nameKey] || '').trim();
            if (!rawName) return;
            const name = rawName.replace(/[\s　]+/g, ' ');
            indexToName[i] = name;
            // 最初に見つかった index で newId を固定（processFormResponses と同じ規則）
            if (!nameToNewId[name]) {
                nameToNewId[name] = generateParticipantId(name, i);
            }
            // メタ情報（最新の回答で上書きしていく）
            const gradeStr = gradeKey ? String(row[gradeKey]) : '';
            let grade = '未選択';
            ...
            const hasTablet = tabletKey ? String(row[tabletKey]).includes('希望しない') : false;
            const schoolYear = yearKey ? String(row[yearKey] || '') : '';
            nameToMeta[name] = { grade, hasTablet, schoolYear };
        });

        // 4. 各セッションの参加者を新IDに統合
        const mergeFields = (dst, src) => {
            ['attended', 'rpContent', 'rpScore', 'apContent', 'apScore', 'remarks'].forEach(f => {
                const cur = dst[f];
                const inc = src[f];
                const curEmpty = (cur === '' || cur === undefined || cur === null || cur === false);
                if (curEmpty && (inc !== '' && inc !== undefined && inc !== null && inc !== false)) {
                    dst[f] = inc;
                } else if (f === 'remarks' && typeof inc === 'string' && typeof cur === 'string' && inc.length > cur.length) {
                    dst[f] = inc; // remarks は長い方を優先
                }
            });
            return dst;
        };
        const emptyP = () => ({ attended: true, rpContent: '', rpScore: '', apContent: '', apScore: '', remarks: '' });
        ...
        Object.keys(cloudAppData).forEach(sessionId => {
            const session = cloudAppData[sessionId] || {};
            const newSession = {
                generalHomework: session.generalHomework || '',
                generalNotes: session.generalNotes || '',
                participants: {}
            };
            const participants = session.participants || {};

            Object.keys(participants).forEach(oldId => {
                const pData = participants[oldId] || {};
                let targetId = null;

                if (oldId.startsWith('p_form_')) {
                    targetId = oldId;
                } else if (oldId.startsWith('p_ext_')) {
                    const m = oldId.match(/_(\d+)$/);
                    if (m) {
                        const idx = parseInt(m[1], 10);
                        const name = indexToName[idx];
                        if (name && nameToNewId[name]) {
                            targetId = nameToNewId[name];
                            migratedCount++;
                        }
                    }
                } else if (oldId.startsWith('p_manual_')) {
                    targetId = oldId;
                }

                if (!targetId) {
                    // マッピングできない孤児: そのまま残す（データ保護）
                    newSession.participants[oldId] = pData;
                    orphanCount++;
                    return;
                }

                if (!newSession.participants[targetId]) {
                    newSession.participants[targetId] = emptyP();
                }
                mergeFields(newSession.participants[targetId], pData);
            });

            mergedAppData[sessionId] = newSession;
        });

        // 5. participantsList をフォーム回答から再構築（既存の手動参加者は保持）
        const newParticipantsList = [];
        Object.keys(nameToNewId).forEach(name => {
            const meta = nameToMeta[name] || { grade: '未選択', hasTablet: false, schoolYear: '' };
            newParticipantsList.push({
                id: nameToNewId[name],
                name: name,
                grade: meta.grade,
                hasTablet: meta.hasTablet,
                schoolYear: meta.schoolYear
            });
        });
        // 既存の手動参加者 (p_manual_*) を保持
        participantsList.forEach(p => {
            if (p.id && p.id.startsWith('p_manual_')) {
                if (!newParticipantsList.some(x => x.id === p.id)) {
                    newParticipantsList.push(p);
                }
            }
        });

        // 6. sessionsInfo は現在のローカル値をそのまま採用
        const newSessionsInfo = sessionsInfo.slice();

        showLoading('統合後のデータをクラウドに保存中...');
        ...
        cloudSynced = true;
        renderSidebar();
        if (currentSessionId) renderMainContent();

        hideLoading();
        alert(
            '✅ 統合が完了しました。\n\n' +
            `・統合された旧ID: ${migratedCount} 件\n` +
            `・マッピングできず温存されたID: ${orphanCount} 件\n` +
            `・確定した参加者: ${newParticipantsList.length} 名\n\n` +
            'バックアップファイル "eiken_cloud_backup_*.json" もダウンロードしました。'
        );
    } catch (e) {
        hideLoading();
        console.error('統合エラー:', e);
